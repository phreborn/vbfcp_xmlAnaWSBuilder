...
injectPoint=p01
injPoi=
if [ ${injectTest} -eq 1 ];then injPoi=",mu_ggH_${injectPoint}=0,mu_VBF_${injectPoint}=0";fi
...
for init in ${sequence[@]};do
  ...
  for num in `seq ${init} 1 ${fin}`;do
    ...
    echo "quickFit -f WS${preCfg}/vbf_cp_${dList[${num}]}/vbf_cp_${dList[${num}]}.root -w combWS -d ${dataset} -p mu=1,mu_ggH_SM=0,mu_VBF_SM=0,mu_ggH=1,mu_VBF_RW=1_0_5${injPoi} -n ATLAS_* -o out${preCfg}_statOnly/out_${dList[${num}]}.root --savefitresult 1 --saveWS 1 --minTolerance ${tole}" >> exe_${jobName}.sh
    echo "quickFit -f WS${preCfg}/vbf_cp_${dList[${num}]}/vbf_cp_${dList[${num}]}.root -w combWS -d ${dataset} -p mu=1,mu_ggH_SM=0,mu_VBF_SM=0,mu_ggH=1,mu_VBF_RW=1_0_5${injPoi} -o out${preCfg}_allSys/out_${dList[${num}]}.root --savefitresult 1 --saveWS 1 --minTolerance ${tole}" >> exe_${jobName}.sh
    if [ ${decompSys} -eq 1 ];then
