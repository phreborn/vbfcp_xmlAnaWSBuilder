...
with open("impactDtilde.txt", 'w') as fout:
 for sysname in syslist:
  ...
  for fitType in ['up_post', 'dn_post', 'up_pre', 'dn_pre']:
   ...
   inNLL = dirname + "dNLL_AllCats_allSys_"+sysname+"_"+fitType+".log"
 ...
 #      print '95%CL interval right:', gr.Eval(1.92)
  ...
  fout.write('\n%s %s %s %s %s %s'%(sysname, '0', dtilde[0], dtilde[1], dtilde[2], dtilde[3]))
  print sysname, 0, dtilde[0], dtilde[1], dtilde[2], dtilde[3]
